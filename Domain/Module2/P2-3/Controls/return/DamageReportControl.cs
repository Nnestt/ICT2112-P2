...
public class DamageReportControl : iDamageReportCRUD, iDamageReportQuery
{
    ...
    /// <summary>
    /// Renders description text as HTML — splits off the price breakdown block
    /// into a styled table if present, keeping the original note as a paragraph.
    /// </summary>
    private static string RenderDescription(string text)
    {
        ...
        // Parse breakdown lines: "Price breakdown:\nProduct price: $x\nRepair cost: $x\nFinal cost: $x"
        ...
        return $"{mainText}<div class='breakdown'>" +
    ...
    public byte[] DownloadDamageReport(int damageReportId)
    {
        ...
        // Embed image as base64 if it's a local file path
        var imageHtml = "";
        ...
        if (!string.IsNullOrWhiteSpace(imagePath))
        {
            ...
            if (File.Exists(fullPath))
            {
                ...
                imageHtml = $"<div class='row'><span class='label'>Damage Image:</span></div>" +
...
<style>
  ...
  .section-title {{ font-size: 10px; font-weight: bold; text-transform: uppercase;
                    letter-spacing: 0.08em; color: #999; margin: 16px 0 4px; }}
  .value {{ font-size: 14px; color: #222; }}
  .badge {{ display: inline-block; padding: 3px 12px; border-radius: 12px;
            background: #c0392b; color: white; font-size: 11px; font-weight: bold; }}
  ...
  .print-btn {{ background:#c0392b;color:white;border:none;padding:8px 20px;border-radius:4px;
